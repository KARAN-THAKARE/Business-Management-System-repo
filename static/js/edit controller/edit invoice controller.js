...
app.controller('EDIT_INVOICE_CONTROLLER', function($scope,$window,$http,$log) {
...
//--------------------------------------------------------- SET UP INVOICE VALUE INITIALY START ---------------------------------------------------
  ...
  //--------------------------------------------------------------- FETCH INVOICE NUMBER START ------------------------------------------------------
     ...
     $http({
     method : "GET",
     url : "/FetchInvoiceData.do/"+InvoiceID,
      }).then(function mySuccess(response) {
        for (let value of Object.values(response.data)) {
         $scope.Invoice.InvoiceNumber=InvoiceID;
          $scope.Invoice.it=value.Invoice_Type;
         $scope.Invoice.InvoiceDate = new Date(value.Invoice_Date);
         $scope.Invoice.POS=value.POS;
         $scope.Invoice.SoldBy=value.Sold_By;

         /* client dataset */
         $scope.Invoice.ClientName=value.ClientName;
         $scope.Invoice.contactNo=value.contact_no;
         $scope.Invoice.Address=value.address;
         $scope.Invoice.state=value.state;
         $scope.Invoice.city=value.city;
         $scope.Invoice.Pincode=value.pincode;
         $scope.Invoice.PAN=value.PAN_NO;
         $scope.Invoice.Email=value.email;

         //payment dataset
         if(value.Apply_Discount=='true'){
            $scope.Invoice.Apply_Discount=true;
         }else{  $scope.Invoice.Apply_Discount=false;}

         $scope.Invoice.Disc_in_percentage=value.Dis_in_percent;
         $scope.Invoice.Disc_in_amt=value.Dis_in_amount;
         $scope.Invoice.SubTotal=value.Sub_Total_Amount;
         $scope.Invoice.TotalAmount=value.Total_Amount;

         if(value.Apply_Shipping=='true'){
            $scope.Invoice.Apply_Shipping=true;
         }else{  $scope.Invoice.Apply_Shipping=false;}

         $scope.Invoice.ShippingAmt=value.Shipping_Amount;

         if(value.Apply_EMI=='true'){
            $scope.Invoice.Apply_EMI=true;
         }else{  $scope.Invoice.Apply_EMI=false;}


         $scope.Invoice.EMI_Months=value.EMI_Months;
         $scope.Invoice.EMI_PERCENT=value.EMI_Percent;
         $scope.Invoice.DP_in_Percent=value.DP_in_Percent;
         $scope.Invoice.DP_in_Amount=value.DP_in_Amount;


         $scope.Invoice.Initialization_Flag=true;
         $scope.Invoice.Previous_Amount_Paid=value.Amount_Paid;


         $scope.Invoice.Invoice_Created_Date=value.Invoice_Created_Date;
         $scope.TODAYS_DAY=value.Todays_Date;
         if($scope.Invoice.Invoice_Created_Date==$scope.TODAYS_DAY){
            $scope.Disabled_Field=false;
            $scope.Invoice.Update_Type='full';
         }else{
             $scope.Disabled_Field=true;
             $scope.Invoice.Update_Type='half';
         }
        ...
        Notiflix.Report.Warning('Oops !','Hey user, Software facing issue while faching Invoice Data. Please try again .If issue is persists , please contact service provider.','Try Again');
      });
   }
   FetchInvoice();
 //========================================== END END END END ============================================
...
//set amount paid on initial value
  if($scope.Invoice.Initialization_Flag){
    $scope.Invoice.Amount_Paid=$scope.Invoice.Previous_Amount_Paid;
    $scope.Invoice.Initialization_Flag=false;
  }
...
$scope.ZERO_AMOUNT_PAID_FIELD=function(Invoice){
  $scope.Invoice.Amount_Paid=null;
...
//@Submit Invoice
  ...
  Notiflix.Loading.Pulse('Please wait ...');
   $http.post('/UpdateInvoice.do',Invoice).
      success(function(results) {
  ...
  $scope.Submit_Validation=function(Invoice){
   ...
   if(Invoice.Apply_EMI){
         if(typeof Invoice.EMI_Months === 'undefined' || Invoice.EMI_Months<0 || Invoice.EMI_Months === null || Invoice.EMI_Months === ''  || Invoice.EMI_Months === 0){
