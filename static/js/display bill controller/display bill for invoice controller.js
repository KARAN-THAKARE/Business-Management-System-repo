...
app.controller('DISPLAY_BILL-INVOICE_CONTROLLER', function($scope,$window,$http,$log) {

$scope.SwitchButton=false
$scope.ShowInvoice=true;
$scope.SwitchMesg="Click here to Switch Invoice to Detailed Format .";
var Invoice_NO =$('#Invoice_NO').val();
$scope.Invoice_ID=Invoice_NO;
var TYPE =$('#Type').val();
$scope.InvoiceType="Short";
$scope.PDF_GENRATOR_URL='Download_PDF_For_Invoice/'+Invoice_NO+'/'+'Short';
...
$scope.FETCH_INVOICE_INFO_FOR_INVOICE=function(Invoice_NO) {
    ...
    success(function(results) {
       ...
       for (let value of Object.values($scope.Invoice_Info)) {
        Message='Hey user, You have successfully created new Invoice for Mr./Ms. ' + value.ClientName ;
         }
         if(TYPE=='new'){
...
$scope.FETCH_INVOICE_INFO_FOR_INVOICE(Invoice_NO,Type);
...
$scope.FETCH_PAYMENT_MODE_INFO_FOR_INVOICE=function(Purchase_NO) {
    URL="/FETCH_PAYMENT_MODE_INFORMATION_FOR_INVOICE.do/"+Invoice_NO
    $http.post(URL).
    success(function(results) {
       $scope.PaymentInfo=results;
        if(Object.keys($scope.PaymentInfo).length>0){
            $scope.PaymentModePresent=true;
        }else{
          $scope.PaymentModePresent=false;
        }
    }).
    error(function(error) {
...
$scope.SwitchInvoice=function(){
  if($scope.SwitchButton){
    ...
    $scope.PDF_GENRATOR_URL='Download_PDF_For_Invoice/'+Invoice_NO+'/'+'Detailed';
  }
...
Notiflix.Confirm.Show(
 ...
 function(){
   ...
   URL="/DELETE_INVOICE_BILL.do/"+Invoice_NO
     $http.post(URL).
     success(function(results) {
       if(results=='true'){
     ...
     error(function(error) {
        Notiflix.Notify.Failure('Oops ! Failed To Delete.');
        console.log(" DELETE_INVOICE_BILL -----> Error :- ",error);
     });
...
$scope.ReturnInvoice= function() {
   ...
   URL="/RETURN_INVOICE.do/"+Invoice_NO+"/"+$scope.remark
     $http.post(URL).
     success(function(results) {
       if(results=='true'){
         $('#ReturnInvoiceRemark').modal('hide');
     ...
     error(function(error) {
        Notiflix.Notify.Failure('Oops ! Failed To Return.');
        console.log(" RETURN_INVOICE -----> Error :- ",error);
     });
...
$scope.PrintClientInvoice=function() {
  Notiflix.Loading.Pulse('Printing ...');
  $http({
  method : "GET",
  url : "/PrintClientInvoice.do/"+$scope.Invoice_ID+"/"+$scope.InvoiceType,
   }).then(function mySuccess(response) {
