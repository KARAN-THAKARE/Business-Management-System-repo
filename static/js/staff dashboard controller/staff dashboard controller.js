...
app.controller('STAFF_DASHBOARD_CONTROLLER', function($scope,$window,$http,$log) {
  $scope.GENDERS = ["Male", "Female", "Other"];
  $scope.Designation=["Manager", "Worker", "Other"];
  $scope.DOCUMENTTYPE = ["Aadhar Card", "Pan Card", "Voter Card","Driving License"];

  $scope.DATA_PRESENT_FOR_UPDATE=true;
  $scope.staff={};

  $scope.addstaffbutton=false;

  $scope.SET_UP_DATA = function(staff) {
  $scope.staff.FN='';
  $scope.staff.LN='';
  $scope.staff.address='';
  $scope.staff.state='';
  $scope.staff.city='';
  $scope.staff.pincode='';
  $scope.staff.contactNumber='';
  $scope.staff.Designation='';
  }
  $scope.SET_UP_DATA($scope.staff);
  ...
  SearchStaffInfoTable = function(CLIENT_ID) {
  Search={}
  $scope.STAFF_DATA={}
  Search.ID=CLIENT_ID;
  $http.post('/GetStaffData.do',Search).
    success(function(results) {
      $scope.STAFF_DATA=results;
      if(Object.keys($scope.STAFF_DATA).length>0){
      document.getElementById("thirdtab").click();
      $scope.DATA_PRESENT_FOR_UPDATE=false;
      Notiflix.Notify.Success('Staff found Successfully !');
      }
      else {
          Notiflix.Notify.Failure('Oops ! Staff data not found .');
      }
  ...
  //Fetch CLIENT DATA LIST
    FetchStaffDataList=function(){
    var StaffList=[]
    $http({
    method : "GET",
    url : "/StaffListforSearch.do",
     }).then(function mySuccess(response) {
       $scope.FetchStaffDataList = response.data;
       for (var key in $scope.FetchStaffDataList) {
            StaffList.push($scope.FetchStaffDataList[key].Staff_Name);
            StaffList.push($scope.FetchStaffDataList[key].Contact_No.toString());
            }
      autocomplete(document.getElementById("searchTab"), StaffList);
     }, function myError(response) {
...
FetchStaffDataList();
  ...
  //SUBMIT STAFF FORM
  $scope.SubmitStaffForm = function(staff) {
  $scope.addstaffbutton=true;
if($scope.Submit_Validation(staff)){

  $http.post('/AddStaff.do',staff).
    success(function(results) {
      if(results=="Success"){
      $scope.staff={};
      document.getElementById("ADD_STAFF_FORM").reset();
      Notiflix.Report.Success('Successfully Done !','Hey user, New staff is successfully added and joined with us , you can see and manage staff through STAFF DASHBOARD .','Done');
      $scope.addstaffbutton=false;

        //If new staff is successfully added then fetch latest data
           FetchStaffDataList();
        //END END

    }else{
        $scope.addstaffbutton=false;
        Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while adding staff information  . Please try again with validation . If problem is shown for a while please contact service provider .','Try Again');
    }
    }).
    error(function(error) {
      $scope.addstaffbutton=false;
  ...
  $scope.Submit_Validation=function(staff){
   if(typeof staff.FN === 'undefined' || staff.FN === '' || staff.FN == null){
         Notiflix.Confirm.Show('Oops !','Hey user , Please enter First Name. ','Try Again');
         return false;
   }
   if(typeof staff.LN === 'undefined' || staff.LN === '' || staff.LN == null){
         Notiflix.Confirm.Show('Oops !','Hey user , Please enter Last Name. ','Try Again');
         return false;
   }
   if(typeof staff.address === 'undefined' || staff.address === '' || staff.address == null){
         Notiflix.Confirm.Show('Oops !','Hey user , Please select staff Full Billing Address. ','Try Again');
         return false;
   }
   if(typeof staff.state === 'undefined' || staff.state === '' || staff.state == null){
         Notiflix.Confirm.Show('Oops !','Hey user , Please select State of staff.','Try Again');
         return false;
   }
   if(typeof staff.city === 'undefined' || staff.city === '' || staff.city == null){
         Notiflix.Confirm.Show('Oops !','Hey user , Please select city of staff.','Try Again');
         return false;
   }
   if(typeof staff.pincode === 'undefined' || staff.pincode === '' || staff.pincode == null){
         Notiflix.Confirm.Show('Oops !','Hey user , Please enter the staff Pincode .','Try Again');
         return false;
   }
   if(typeof staff.contactNumber === 'undefined' || staff.contactNumber === '' || staff.contactNumber == null){
         Notiflix.Confirm.Show('Oops !','Hey user , Please enter the staff contact number.','Try Again');
         return false;
   }
   if(typeof staff.Designation === 'undefined' || staff.Designation === '' || staff.Designation == null){
         Notiflix.Confirm.Show('Oops !','Hey user , Please select the staff Designation.','Try Again');
         return false;
   }
   if(typeof staff.SA === 'undefined' || staff.SA === '' || staff.SA == null){
         Notiflix.Confirm.Show('Oops !','Hey user , Please enter salary amount.','Try Again');
         return false;
   }
  return true;
  }
...
//search Staff data
$scope.SearchStaffInfo = function(STAFF_DATA) {
...
$scope.Updation_Validation=function(staff){
 if(typeof staff.First_Name === 'undefined' || staff.First_Name === ''  || staff.First_Name == null){
       Notiflix.Confirm.Show('Oops !','Hey user , Please enter First Name. ','Try Again');
       return false;
 }
 if(typeof staff.Last_Name === 'undefined' || staff.Last_Name === ''  || staff.Last_Name == null){
       Notiflix.Confirm.Show('Oops !','Hey user , Please enter Last Name. ','Try Again');
       return false;
 }
 if(typeof staff.Address === 'undefined' || staff.Address === ''  || staff.Address == null){
 ...
 if(typeof staff.Contact_No === 'undefined' || staff.Contact_No === '' || staff.Contact_No == null){
       Notiflix.Confirm.Show('Oops !','Hey user , Please enter the staff contact number.','Try Again');
       return false;
 }
 if(typeof staff.Designation === 'undefined' || staff.Designation === '' || staff.Designation == null){
...
$scope.Update_Staff_Information = function(STAFF_DATA) {
var DOB=document.getElementById("staff_DOB_UPDATE").value;
STAFF_DATA["DOB"]=DOB;
if($scope.Updation_Validation(STAFF_DATA)){
$http.post('/UpdateStaffInfo.do',STAFF_DATA).
  success(function(results) {
    if(results=="Success"){
    document.getElementById("UPDATE_DELETE_STAFF_INFO_SEARCH").reset();
    document.getElementById("UPDATE_DELETE_STAFF_FORM").reset();
...
$scope.Delete_Staff_Information = function(STAFF_DATA) {
$http.post('/DeleteStaffInfo.do',STAFF_DATA).
  success(function(results) {
    if(results=="Success"){
    document.getElementById("UPDATE_DELETE_STAFF_INFO_SEARCH").reset();
    document.getElementById("UPDATE_DELETE_STAFF_FORM").reset();
...
DeleteButtonTable = function(STAFF_ID) {
DATA={}
DATA['Staff_ID']=STAFF_ID;

Notiflix.Confirm.Show(
  "Confirmation Box",
  "Are you sure you want to proceed ? If you press 'Yes' then selected Staff data will be lost .",
  false,
  false,
  function(){
    $http.post('/DeleteButtonTableForStaff.do',DATA).
      success(function(results) {
        if(results=="Success"){
        Notiflix.Notify.Success('Successfully Deleted !');
