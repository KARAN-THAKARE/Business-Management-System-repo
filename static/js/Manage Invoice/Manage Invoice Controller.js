...
app.controller('MANAGE~INVOICE~CONTROLLER', function($scope,$window,$http,$log) {

SEND_INVOICE_ON_MAIL=function(ID,ClientName) {

    Notiflix.Loading.Pulse('Sending...');
    URL="/SEND_INVOICE_ON_MAIL.do/"+ID
    $http.post(URL).
    success(function(results) {
    if(results.isSuccess=='true'){
      var Message ="Hey user,Invoice has been successfully sent on " +  ClientName + "'s Email ID as per your request."
      Notiflix.Loading.Remove();
      Notiflix.Report.Success('Successfully Sent !',Message,'Okey');
    }else{
      var Message ="Hey user, Software is trying to send Invoice on "+ ClientName + "'s Email ID but " + results.FrontEndMesg
      Notiflix.Loading.Remove();
