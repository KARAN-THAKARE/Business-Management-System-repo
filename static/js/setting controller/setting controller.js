...
//_________________________________________________________________________________________________________________
//_____________________________________  SETTING CONTROLLER MODULE  ________________________________________________
//_________________________________________________________________________________________________________________



app.controller('SETTING~CONTROLLER', function($scope,$window,$http,$log) {
...
//-----------------------------------------------------------------------------------------------------------------------
  //Fetch User Data
    ...
    $http({
    method : "GET",
    url : "/FetchUserProfileDetails.do",
     }).then(function mySuccess(response) {
...
//Fetch Software Information
  Fetch_Software_Information=function(){
    $scope.UserData={};
  $http({
  method : "GET",
  url : "/FetchSoftwareInformation.do",
   }).then(function mySuccess(response) {
     $scope.SoftwareInfo = response.data;
   }, function myError(response) {
     console.log(response);
   });
}
Fetch_Software_Information();


//Update User Data
  $scope.Validation_Update_User_Data=function(UserProfile){
    if(typeof UserProfile.FN === 'undefined' || UserProfile.FN === '' || UserProfile.FN === null){
       Notiflix.Confirm.Show('Oops !','Hey user , Please enter First Name. ','Try Again');
      return false;
     }
     if(typeof UserProfile.MN === 'undefined' || UserProfile.MN === '' || UserProfile.MN === null){
        Notiflix.Confirm.Show('Oops !','Hey user , Please enter Middle Name. ','Try Again');
       return false;
      }
      if(typeof UserProfile.LN === 'undefined' || UserProfile.LN === '' || UserProfile.LN === null){
         Notiflix.Confirm.Show('Oops !','Hey user , Please enter Last Name. ','Try Again');
        return false;
       }
       if(typeof UserProfile.Address === 'undefined' || UserProfile.Address === '' || UserProfile.Address === null){
          Notiflix.Confirm.Show('Oops !','Hey user , Please enter Full Address. ','Try Again');
         return false;
        }
        if(typeof UserProfile.City === 'undefined' || UserProfile.City === '' || UserProfile.City === null){
           Notiflix.Confirm.Show('Oops !','Hey user , Please enter City','Try Again');
          return false;
         }
         if(typeof UserProfile.State === 'undefined' || UserProfile.State === '' || UserProfile.State === null){
            Notiflix.Confirm.Show('Oops !','Hey user , Please enter State.','Try Again');
           return false;
          }
          if(typeof UserProfile.ZipCode === 'undefined' || UserProfile.ZipCode === '' || UserProfile.ZipCode === null){
             Notiflix.Confirm.Show('Oops !','Hey user , Please enter Zip code.','Try Again');
            return false;
           }
           if(typeof UserProfile.PhoneNo === 'undefined' || UserProfile.PhoneNo === '' || UserProfile.PhoneNo === null){
              Notiflix.Confirm.Show('Oops !','Hey user , Please enter Phone Number.','Try Again');
             return false;
            }
            if(typeof UserProfile.Email === 'undefined' || UserProfile.Email === '' || UserProfile.Email === null){
               Notiflix.Confirm.Show('Oops !','Hey user , Please enter your Email ID.','Try Again');
              return false;
             }
            return true;
  }
  ...
  $scope.UpdateUserData=function(UserProfile){
  if($scope.Validation_Update_User_Data(UserProfile)){
    $http.post('/UpdateUserProfileDetails.do',UserProfile).
      success(function(results) {
        if(results=="Success"){
            $scope.SaveButton=true;
            Notiflix.Report.Success('Successfully Update !','Hey user, User Details has been Successfully updated.','Done');
        }else{
          Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while updating user details. Please try again with validation .If issue is persists , please contact service provider ','Try Again');
        }
      }).
      error(function(error) {
        Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while updating user details. Please try again with validation .If issue is persists , please contact service provider ','Try Again');
...
//================================================= Setting --> User Profile End ============================================
//---------------------------------------------------------------------------------------------------------------------------


//================================================= Setting --> Store Profile ============================================
//-----------------------------------------------------------------------------------------------------------------------
  //Fetch Store Data
...
//Update Shope Data
  $scope.Validation_Update_Store_Data=function(ShopProfile){
    if(typeof ShopProfile.Name === 'undefined' || ShopProfile.Name === '' || ShopProfile.Name === null){
       Notiflix.Confirm.Show('Oops !','Hey user , Please enter Shop Name. ','Try Again');
      return false;
     }
       if(typeof ShopProfile.Address === 'undefined' || ShopProfile.Address === '' || ShopProfile.Address === null){
          Notiflix.Confirm.Show('Oops !','Hey user , Please enter Full Address of shop. ','Try Again');
         return false;
        }
        if(typeof ShopProfile.City === 'undefined' || ShopProfile.City === '' || ShopProfile.City === null){
           Notiflix.Confirm.Show('Oops !','Hey user , Please enter City','Try Again');
          return false;
         }
         if(typeof ShopProfile.State === 'undefined' || ShopProfile.State === '' || ShopProfile.State === null){
            Notiflix.Confirm.Show('Oops !','Hey user , Please enter State.','Try Again');
           return false;
          }
          if(typeof ShopProfile.ZipCode === 'undefined' || ShopProfile.ZipCode === '' || ShopProfile.ZipCode === null){
             Notiflix.Confirm.Show('Oops !','Hey user , Please enter Zip code.','Try Again');
            return false;
           }
           if(typeof ShopProfile.Contact_No_1 === 'undefined' || ShopProfile.Contact_No_1 === '' || ShopProfile.Contact_No_1 === null){
              Notiflix.Confirm.Show('Oops !','Hey user , Please enter primary Contact Number.','Try Again');
             return false;
            }
            if(typeof ShopProfile.Contact_No_2 === 'undefined' || ShopProfile.Contact_No_2 === '' || ShopProfile.Contact_No_2 === null){
               Notiflix.Confirm.Show('Oops !','Hey user , Please enter secondary Contact Number.','Try Again');
              return false;
             }
            if(typeof ShopProfile.Email === 'undefined' || ShopProfile.Email === '' || ShopProfile.Email === null){
               Notiflix.Confirm.Show('Oops !','Hey user , Please enter shop Email ID.','Try Again');
              return false;
             }
             if(typeof ShopProfile.GSTIN === 'undefined' || ShopProfile.GSTIN === '' || ShopProfile.GSTIN === null){
                Notiflix.Confirm.Show('Oops !','Hey user , Please enter shop GSTIN Number.','Try Again');
               return false;
              }
            return true;
  }
  ...
  $scope.UpdateStoreData=function(ShopProfile){
  if($scope.Validation_Update_Store_Data(ShopProfile)){
    $http.post('/UpdateStoreProfileDetails.do',ShopProfile).
      success(function(results) {
        if(results=="Success"){
            $scope.SaveButton=true;
            Notiflix.Report.Success('Successfully Update !','Hey user, Shop Details has been Successfully updated.','Done');
        }else{
          Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while updating Shop details. Please try again with validation .If issue is persists , please contact service provider ','Try Again');
        }
      }).
      error(function(error) {
        Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while updating Shop details. Please try again with validation .If issue is persists , please contact service provider ','Try Again');
...
//----------------------------------------------- START RENEWAL START ---------------------------------------
//***********************************************************************************************************
$scope.ExtenedSubscription=function(){
 if($scope.Renew_Validation()){
   $http.post("/ExtenedSubscription.do",$scope.AuthKey).
   success(function(results) {
     if(results.isSuccess){
           if(results.StatusCode=='00x020ATIVE'){
             $scope.AuthKey={};
             Fetch_Software_Information();
             Notiflix.Report.Success('Congratulations !!!','Hey user, Your subscription has been successfully renewed . You can enjoy our service until next expiration date.');
           ...
           }else if(results.StatusCode=='00x0F1SHTALL'){
           ...
           if(results.StatusCode=='00x0F2A'){
            Notiflix.Report.Warning('Warning !','You Have entered wrong Authentication Key (Error Code : 00x0F2A). Please contact service provider and enter provided authentication key only otherwise system will get block.','Try Again');
          }else{
...
//Fetch Terms and CONDITIONS Data
  FETCH_TERMS_AND_CONDITIONS=function(){
  $scope.StoreData={};
  $http({
  method : "GET",
  url : "/FetchTermsAndCondition.do",
   }).then(function mySuccess(response) {
...
FETCH_TERMS_AND_CONDITIONS();
  ...
  $scope.AddTerms=function(Terms_Condition){
    $scope.Terms_And_Condition={};
    $scope.Terms_And_Condition.terms=Terms_Condition;

    if($scope.Validation_Terms_And_Condition(Terms_Condition)){
      $http.post('/AddTerms.do',$scope.Terms_And_Condition).
        success(function(results) {
          if(results=="Success"){
              $('#Add_Terms_And_Condition').modal('hide');
              document.getElementById("TC_Form").reset();
              FETCH_TERMS_AND_CONDITIONS(); //Fetch terms data again
              Notiflix.Report.Success('Successfully Added !','Hey user, New Terms and Conditions has been Successfully added.','Done');
          }else{
            Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while adding New Terms and Conditions. Please try again with validation .If issue persists , please contact service provider ','Try Again');
          }
        }).
        error(function(error) {
          Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while adding New Terms and Conditions. Please try again with validation .If issue persists , please contact service provider ','Try Again');
...
$scope.Validation_Terms_And_Condition=function(Terms_Condition){
  if(typeof Terms_Condition=== 'undefined' || Terms_Condition=== '' || Terms_Condition === null){
     Notiflix.Confirm.Show('Oops !','Hey user , Please enter Terms and Condition','Try Again');
    return false;
   }
          return true;
}

$scope.UpdateTermsModel=function(TermID,Term){
  $scope.TERM_ID=TermID;
  $scope.Update_Terms_Condition=Term;
  $('#Edit_Terms_And_Condition').modal('show');
}

$scope.UpdateTerms=function(){
  let DATA={};
  DATA.TERM_ID=$scope.TERM_ID;
  DATA.Update_Terms_Condition=$scope.Update_Terms_Condition;
      $http.post('/UpdateTerms.do',DATA).
      success(function(results) {
        if(results=="Success"){
            $('#Edit_Terms_And_Condition').modal('hide');
            FETCH_TERMS_AND_CONDITIONS(); //Fetch terms data again
            Notiflix.Notify.Success('Terms update Successfully !');
        }else{
            Notiflix.Notify.Failure('Oops ! Failed to update.');
          }
      }).
      error(function(error) {
        Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while updating Terms and Conditions. Please try again with validation .If issue persists , please contact service provider ','Try Again');
      });
}

//Delete Terms and Condition
$scope.DeleteTerms=function(TermsID){
  URL='/DeleteTerms.do/'+TermsID
    $http.post(URL).
      success(function(results) {
        if(results=="Success"){
            FETCH_TERMS_AND_CONDITIONS(); //Fetch terms data again
            Notiflix.Notify.Success('Terms delete Successfully !');
        }else{
            Notiflix.Notify.Failure('Oops ! Failed to delete.');
          }
      }).
      error(function(error) {
        Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while deleting Terms and Conditions. Please try again with validation .If issue persists , please contact service provider ','Try Again');
      });
}


//Change Access for terms and condition
$scope.ChangeAccess=function(TermsID,ForWhich,ForWhichStatus){
 if(ForWhichStatus=='true'){
   ForWhichStatus='false';
 }else{
  ForWhichStatus='true';
 }

  URL='/UpdateTermsAccess.do/'+TermsID+'/'+ForWhich+'/'+ForWhichStatus
    $http.post(URL).
      success(function(results) {
        if(results=="Success"){
          console.log("Successfully update access for terms and condition");
        }else{
          Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while updating access for Terms and Conditions. Please try again .If issue persists , please contact service provider ','Try Again');
          }
      }).
      error(function(error) {
        Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while updating access for Terms and Conditions. Please try again .If issue persists , please contact service provider ','Try Again');
...
//================================================= Setting --> Terms and Condition End ===================================
//--------------------------------------------------------------------------------------------------------------------------


//==================================================== HANDLE KEYWORD EVENT ===============================================
//--------------------------------------------------------------------------------------------------------------------------
...
//_________________________________________________________________________________________________________________
//____________________________________ END SETTING CONTROLLER MODULE  END _________________________________________
//_________________________________________________________________________________________________________________
