...
app.controller('SUPLLIER_DASHBOARD_CONTROLLER', function($scope,$window,$http,$log) {
  ...
  $scope.SET_UP_DATA = function(supplier) {
  $scope.supplier.FN='';
  $scope.supplier.LN='';
  $scope.supplier.address='';
  $scope.supplier.state='';
  $scope.supplier.city='';
  $scope.supplier.pincode='';
  $scope.supplier.contactNumber='';
  }
  $scope.SET_UP_DATA($scope.supplier);



  //EDIT BUTTON OF TABLE
  SearchSupplierInfoTable = function(SUPPLIER_ID) {
  Search={}
  $scope.SUPPLIER_DATA={}
  Search.ID=SUPPLIER_ID;
  $http.post('/GetSupplierData.do',Search).
    success(function(results) {
      $scope.SUPPLIER_DATA=results;
      if(Object.keys($scope.SUPPLIER_DATA).length>0){
      document.getElementById("thirdtab").click();
      $scope.DATA_PRESENT_FOR_UPDATE=false;
      Notiflix.Notify.Success('Supplier found Successfully !');
      }
      else {
          Notiflix.Notify.Failure('Oops ! Supplier data not found .');
      }
  ...
  if($scope.Submit_Validation(supplier)){
  $http.post('/AddSupplier.do',supplier).
    success(function(results) {
      if(results=="Success"){
      $scope.supplier={};
      document.getElementById("SUPPLIER_DATA_SUBMITTED").reset();
      ...
      $scope.addsupplierbutton=false;
        ...
        //If new supplier is successfully added then fetch latest data
          $http({
          method : "GET",
          url : "/SupplierListforSearch.do",
           }).then(function mySuccess(response) {
             var SupplierList2=[];
             $scope.FetchSupplierDataList2 = response.data;
             for (var key in $scope.FetchSupplierDataList2) {
                  SupplierList2.push($scope.FetchSupplierDataList2[key].Supplier_Name);
                  SupplierList2.push($scope.FetchSupplierDataList2[key].Contact_No.toString());
                  }
            autocomplete(document.getElementById("searchTab"), SupplierList2);
           }, function myError(response) {
        ...
        $scope.addsupplierbutton=false;
        Notiflix.Report.Failure('Oops ! Failed .','Hey user, Software facing some issue while adding supplier information  . Please try again with validation .If issue is persists , please contact service provider.','Try Again');
    }
    }).
    error(function(error) {
      $scope.addsupplierbutton=false;
  ...
  $scope.Submit_Validation=function(supplier){
  ...
  //Fetch SUPPLIER DATA LIST
    $scope.FetchSupplierDataList=function(){
    var SupplierList=[]
    $http({
    method : "GET",
    url : "/SupplierListforSearch.do",
      ...
      autocomplete(document.getElementById("searchTab"), SupplierList);
     }, function myError(response) {
       console.log(response);
     });
  }
$scope.FetchSupplierDataList();


//search supplier data
$scope.SearchSupplierInfo = function(SUPPLIER_DATA) {
...
$http.post('/GetSupplierData.do',Search).
  success(function(results) {
    $scope.SUPPLIER_DATA=results;
    if(Object.keys($scope.SUPPLIER_DATA).length>0){
    Notiflix.Notify.Success('Supplier found Successfullyv !');
    $scope.DATA_PRESENT_FOR_UPDATE=false;
    }
    else {
        $scope.DATA_PRESENT_FOR_UPDATE=true;
...
$scope.Update_Supplier_Information = function(SUPPLIER_DATA) {
var DOB=document.getElementById("supplier_DOB_UPDATE").value;
SUPPLIER_DATA["DOB"]=DOB;
$http.post('/UpdateSupplierInfo.do',SUPPLIER_DATA).
  success(function(results) {
    if(results=="Success"){
    document.getElementById("UPDATE_DELETE_SUPPLIER_INFO_SEARCH").reset();
    document.getElementById("UPDATE_DELETE_FORM").reset();
    Notiflix.Notify.Success('Successfully Updated !');
    $scope.DATA_PRESENT_FOR_UPDATE=true;
    $scope.SUPPLIER_DATA={}
    //---------------------------UpdateSupplierInfo.do END HERE IF SUCCESS------------------------------------//
...
$scope.Delete_Supplier_Information = function(SUPPLIER_DATA) {
$http.post('/DeleteSupplierInfo.do',SUPPLIER_DATA).
  success(function(results) {
    if(results=="Success"){
    document.getElementById("UPDATE_DELETE_SUPPLIER_INFO_SEARCH").reset();
    document.getElementById("UPDATE_DELETE_FORM").reset();
    ...
    //---------------To Make fresh suggestion list as it may possible user change name also-----------------//
                         var SupplierList=[]
                         $http({
                         method : "GET",
                         url : "/SupplierListforSearch.do",
...
DeleteButtonTable = function(SUPPLIER_ID) {
DATA={}
DATA['Supplier_ID']=SUPPLIER_ID;

Notiflix.Confirm.Show(
  "Confirmation Box",
  "Are you sure you want to proceed ? If you press 'Yes' then selected supplier data will be lost .",
  false,
  false,
  function(){
    $http.post('/DeleteButtonTableForSupplier.do',DATA).
      success(function(results) {
        if(results=="Success"){
        Notiflix.Notify.Success('Successfully Deleted !');
